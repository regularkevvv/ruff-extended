...
pub fn all_symbols<'db>(
    db: &'db dyn Db,
    importing_from: AnalysisFile<'db>,
    query: &QueryPattern,
    ...
    let is_typing_extensions_available = file.is_stub(db)
        || resolve_real_shadowable_module(db, importing_from.program_file(db), &typing_extensions)
            .is_some();
    ...
    let results = all_modules(db, program.resolver(db))
        ...
        .map_with_db(db, |db, module| {
            ...
            for (_, symbol) in symbols_for_file_global_only(db, analysis_file).search(query) {
