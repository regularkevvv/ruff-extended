...
pub fn check_types(db: &dyn Db, file: ProgramFile<'_>) -> Vec<Diagnostic> {
    ...
    diagnostics.extend_diagnostics(plugin_project_index_diagnostics_for_file(
        db,
        env,
        file.file(db),
    ));
...
impl<'db> Type<'db> {
    ...
    fn plugin_replacement_instance_member(
        ...
        match self {
            Type::NominalInstance(instance) => instance
                .class(db, env)
                .plugin_replacement_instance_member(db, env, name, existing_ty),
            Type::NewTypeInstance(newtype) => newtype
                .concrete_base_type(db)
                .plugin_replacement_instance_member(db, env, name, existing_ty),
    ...
    fn plugin_annotated_instance_member(
        ...
        annotated
            ...
            .plugin_annotated_instance_member(db, env, name, self)
    }
    ...
    fn plugin_instance_assignment_member(
        ...
        match self {
            ...
            Type::NewTypeInstance(newtype) => newtype
                .concrete_base_type(db)
                .plugin_instance_assignment_member(db, env, name),
            Type::TypeAlias(alias) => alias
                .value_type(db)
                .plugin_instance_assignment_member(db, env, name),
            _ => None,
    ...
    fn plugin_contributed_instance_assignment_member(
        ...
        match self {
            Type::NominalInstance(instance) => instance
                .class(db, env)
                .plugin_contributed_instance_assignment_member(db, env, name),
            Type::NewTypeInstance(newtype) => newtype
                .concrete_base_type(db)
                .plugin_contributed_instance_assignment_member(db, env, name),
            Type::TypeAlias(alias) => alias
                ...
                Type::KnownInstance(KnownInstanceType::Annotated(annotated)) => {
                        ...
                        this.plugin_annotated_instance_member(db, env, name_str)
                    {
                        return promote_inferred_attribute_class_literals(
                            db,
                            env,
                            this.fallback_to_getattr(db, env, name, plugin_member, policy),
                        );
                    }
        ...
        match self {
            ...
            Type::KnownInstance(known_instance) => match known_instance {
                ...
                KnownInstanceType::Annotated(ty) => {
                    ty.base(db).find_legacy_typevars_impl(
                        db,
                        env,
                        binding_context,
                        typevars,
                    ...
                    for metadata in ty.metadata(db) {
                        metadata.find_legacy_typevars_impl(
                            db,
                            env,
                            binding_context,
                            typevars,
