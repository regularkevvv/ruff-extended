...
pub(crate) fn class_pattern_positional_result<'db>(
    ...
    match class_match_args_type(db, program, class) {
        ...
        ClassMatchArgs::Defined { member_type, .. } => {
            let match_args = member_type.resolve_type_alias(db, program);
            ...
            match class_ty {
                Type::ClassLiteral(class) => {
                    if class_pattern_is_exhaustive(db, program, class, resolved_subject_ty, kind) {
                        ...
                        if !class_pattern_is_exhaustive(db, program, class, top_subject_ty, kind) {
                            return subject_ty;
                        }
