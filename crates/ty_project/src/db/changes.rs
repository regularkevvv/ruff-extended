...
impl ProjectDatabase {
    ...
    pub fn apply_changes(&mut self, changes: &[ChangeEvent]) -> ChangeResult {
        ...
        let configuration_paths = ConfigurationPaths::from_metadata(
...
impl ConfigurationPaths {
    fn from_metadata(metadata: &ProjectMetadata, plugin_reload_paths: &[SystemPathBuf]) -> Self {
        let mut extra = metadata
            .extra_configuration_paths()
            .map(SystemPath::to_path_buf)
            .collect::<Vec<_>>();
        extra.extend_from_slice(plugin_reload_paths);
