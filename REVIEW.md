...
- **Fixed — nested generator materialization:** Independent review found that an outer materialization replaced the stored polarity of an already-static protocol, making `Bottom[Top[G]]` disagree between `__next__` and `yield from`; remapping now preserves the inner materialization polarity.
- **Fixed — mixed materialized meta-type relations:** Independent review found that `type(P)` dropped protocol metadata and compared nominally with `type(Top[P])`, making class-side union simplification order-dependent; when either side is materialized, class-backed protocol meta-types now compare their interfaces.
- **Rejected — property-alias specialization equivalence:** Repeated review reproduced the same property-alias equivalence failure on detached `main`, so it is pre-existing and outside this branch's scope.
- **Fixed — mdtest contract quality:** The added coverage is split into focused sections with plain prose, redundant baseline assertions and reveals were removed, and behavior-change cases were checked against detached `main`; cases that pass on `main` are retained only as explicit preservation guards for the new representation.
